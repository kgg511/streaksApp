...
String modeName =
    "Add Habit"; //the name for the bottom button/title. Either Add Habit/Edit Habit

//global method? It's variables are used in multiple classes
void addMode() {
  //turns it back to add mode
  habitTextBox = "Habit";
            ...
            BottomButton(
              ...
              onTap: () async {
                ...
                if (!edit) {
                  ...
                  int time_s = (new DateTime(
                          a.year, a.month, a.day, 0, 0, 0, 0, 0))
                      .millisecondsSinceEpoch; // the time started and the id
                  Streak s = Streak(
                      id: a.millisecondsSinceEpoch,
                      length: 0,
                      name: habitName,
                      start: time_s,
                      col: screenPickerColor.value); //make streak object
                  List<Streak> streakTest = [
                    s
                  ]; //put streak in a list for the insert method. Fix later
                  bool insert = await handler
                      .insertStreak(streakTest); //true if successful

                  if (!insert) {
                    showDialog<String>(
                      context: context,
                      builder: (BuildContext context) => errorMessage(context),
                    );
                  ...
                  setState(() async {
                    bool edit = await handler.updateName(
                        habitName, screenPickerColor.value, editId);
                    ...
                    if (!edit) {
                      ...
                      showDialog<String>(
                        ...
                        builder: (BuildContext context) => errorMessage(
                            context), //errorMessage is the coded pop-up message
                      );
                    } else {
                      //else if it was successfully changed, return to adding mode & leave editing page
                      addMode();
                      Navigator.pop(context);
                    }
                  });
